    ...
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="text-sm text-slate-500">
                {{ now()->format('M d, Y') }} - {{ auth()->user()->name }}
            </div>
        </div>
    </x-slot>
            ...
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="grid size-11 place-items-center rounded-md bg-emerald-600 text-white">
                        <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.25h5.25a2.25 2.25 0 0 0 0-4.5h-4.5a2.25 2.25 0 0 1 0-4.5H15M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                    </span>
                </div>
            ...
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="grid size-11 place-items-center rounded-md bg-cyan-600 text-white">
                        <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75Zm6.75-4.5c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625Zm6.75-4.5c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                    </span>
                </div>
            ...
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="grid size-11 place-items-center rounded-md bg-amber-500 text-white">
                        <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" /></svg>
                    </span>
                </div>
            ...
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <span class="grid size-11 place-items-center rounded-md bg-red-600 text-white">
                        <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m21 7.5-9-4.5-9 4.5m18 0-9 4.5m9-4.5v9l-9 4.5m0-9L3 7.5m9 4.5v9M3 7.5v9l9 4.5" /></svg>
                    </span>
                </div>
                        ...
                        <div style="min-width: 0; flex: 1; overflow-x: auto; overflow-y: hidden;">
                            <div style="min-width: 720px;">
                                <div class="border-b border-l border-slate-200" style="position: relative; height: 280px;">
                                    <div
                                        style="position: absolute; inset: 0; background-image: linear-gradient(to bottom, rgba(203, 213, 225, .75) 1px, transparent 1px); background-size: 100% 25%;"
                                    ></div>
                                        ...
                                        @foreach ($monthlyCollections as $index => $total)
                                            ...
                                            <div style="position: relative; z-index: 1; display: flex; align-items: end; justify-content: center; height: 100%; min-width: 0;">
                                                <div
                                                    ...
                                                    title="{{ $monthLabels[$index] }}: {{ $money($total) }}"
                                                ></div>
                                            </div>
                                        @endforeach
                                    </div>
            ...
            <section class="rounded-lg border border-slate-200 bg-white shadow-sm">
                ...
                <div class="space-y-3 p-5">
                    <div class="grid grid-cols-3 gap-2 text-center">
                        <div class="rounded-md bg-amber-50 p-3"><p class="text-xl font-bold text-amber-700">{{ $operations['sales_orders']['pending'] }}</p><p class="text-xs font-semibold uppercase text-amber-700">Pending SO</p></div>
                        <div class="rounded-md bg-cyan-50 p-3"><p class="text-xl font-bold text-cyan-700">{{ $operations['sales_orders']['partial'] }}</p><p class="text-xs font-semibold uppercase text-cyan-700">Partial SO</p></div>
                        <div class="rounded-md bg-emerald-50 p-3"><p class="text-xl font-bold text-emerald-700">{{ $operations['sales_orders']['served'] }}</p><p class="text-xs font-semibold uppercase text-emerald-700">Served SO</p></div>
                    </div>
                    <div class="grid grid-cols-3 gap-2 text-center">
                        <div class="rounded-md bg-amber-50 p-3"><p class="text-xl font-bold text-amber-700">{{ $operations['delivery_receipts']['pending'] }}</p><p class="text-xs font-semibold uppercase text-amber-700">Pending D.R</p></div>
                        <div class="rounded-md bg-slate-100 p-3"><p class="text-xl font-bold text-slate-700">{{ $operations['delivery_receipts']['billed'] }}</p><p class="text-xs font-semibold uppercase text-slate-600">Billed D.R</p></div>
                        <div class="rounded-md bg-red-50 p-3"><p class="text-xl font-bold text-red-700">{{ $operations['delivery_receipts']['cancelled'] }}</p><p class="text-xs font-semibold uppercase text-red-700">Cancelled</p></div>
                    </div>
                </div>
            </section>
