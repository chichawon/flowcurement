<div class="space-y-5">
    <section class="erp-panel">
        <div class="erp-panel-header flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h3 class="text-base font-semibold text-slate-950">Sales Invoices</h3>
                <p class="mt-1 text-sm text-slate-500">Bill delivered items from delivery receipts and track invoice balances.</p>
            </div>
            @can('create', \App\Modules\Sales\Models\SalesInvoice::class)
                <a href="{{ route('sales.invoices.create') }}" class="inline-flex items-center justify-center rounded-md bg-slate-950 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">Create Sales Invoice</a>
            @endcan
        </div>
        ...
        <div class="erp-panel-body space-y-4">
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <label class="block lg:col-span-2">
                    ...
                    <input type="search" wire:model.live.debounce.350ms="search" class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm erp-focus-ring" placeholder="Invoice, SO, DR, PO, company">
                </label>
                <label class="block">
                    ...
                    <select wire:model.live="status" class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm erp-focus-ring">
                        ...
                        @foreach (\App\Modules\Sales\Helpers\SalesInvoiceOptions::STATUSES as $option)
                ...
                <table class="w-full table-fixed divide-y divide-slate-200 text-sm">
                    ...
                    <thead class="bg-slate-50 text-xs font-semibold uppercase text-slate-500">
                        <tr>
                            ...
                            <th class="px-3 py-3 text-left">Delivery Receipt</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse ($salesInvoices as $invoice)
                            <tr>
                                ...
                                <td class="px-3 py-3 align-middle"><p class="font-semibold text-slate-950">{{ $invoice->sales_invoice_no }}</p><p class="mt-0.5 text-xs text-slate-500">{{ strtoupper($invoice->currency) }}</p></td>
                                <td class="px-3 py-3 text-center align-middle text-slate-700">{{ $invoice->invoice_date?->format('M d, Y') }}</td>
                                <td class="px-3 py-3 align-middle"><p class="font-medium text-slate-800">{{ $invoice->delivery_receipt_no }}</p><p class="mt-0.5 text-xs text-slate-500">{{ $invoice->sales_order_no }}</p></td>
                                <td class="px-3 py-3 align-middle text-slate-700">{{ $invoice->company_name }}</td>
                                <td class="px-3 py-3 align-middle text-slate-700">{{ $invoice->customer_po ?: 'None' }}</td>
                                <td class="px-3 py-3 text-right align-middle"><p class="font-semibold text-slate-950">{{ number_format((float) $invoice->total_amount, 2) }}</p><p class="mt-0.5 text-xs text-slate-500">Bal {{ number_format((float) $invoice->balance_amount, 2) }}</p></td>
                                <td class="px-3 py-3 text-center align-middle"><x-sales.status-badge :status="$invoice->status" /></td>
                            </tr>
                        @empty
                            <tr><td colspan="8" class="px-4 py-10 text-center text-sm text-slate-500">No sales invoices found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            @if ($salesInvoices->total() > 0)
                @php
                    ...
                    $lastPage = $salesInvoices->lastPage();
                ...
                <div class="flex flex-col gap-3 border-t border-slate-200 pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-slate-500">
                        Showing <span class="font-semibold text-slate-700">{{ $salesInvoices->firstItem() }}</span>
                        to <span class="font-semibold text-slate-700">{{ $salesInvoices->lastItem() }}</span>
                        of <span class="font-semibold text-slate-700">{{ $salesInvoices->total() }}</span> records
                    </p>
                    <div class="flex flex-wrap items-center gap-1">
                        <button type="button" wire:click="previousPage" @disabled($salesInvoices->onFirstPage()) class="inline-flex min-h-9 items-center rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">Previous</button>
                        ...
                        <button type="button" wire:click="nextPage" @disabled(! $salesInvoices->hasMorePages()) class="inline-flex min-h-9 items-center rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">Next</button>
                    </div>
                </div>
            @endif
        </div>
    ...
    <div x-data="{ open: @entangle('showVoidConfirmation').live }" x-show="open" x-transition.opacity x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 sm:px-0" role="dialog" aria-modal="true">
        ...
        <div class="relative w-full max-w-sm rounded-xl bg-white shadow-2xl">
            <div class="border-b border-slate-200 px-5 py-4">
                <h3 class="text-base font-semibold text-slate-950">Cancel sales invoice?</h3>
                <p class="mt-1 text-sm text-slate-500">This invoice will be marked as cancelled.</p>
            </div>
            <div class="px-5 py-4">
                <p class="text-sm text-slate-600">Cancel:</p>
                <p class="mt-1 text-sm font-semibold text-slate-950">{{ $pendingVoidInvoiceNo }}</p>
            </div>
            <div class="flex items-center justify-end gap-2 border-t border-slate-200 px-5 py-4">
                ...
                <button type="button" wire:click="voidConfirmedInvoice" class="rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700">Cancel Invoice</button>
            </div>
        </div>
    </div>
</div>
